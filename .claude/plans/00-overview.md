...
Go のソースコードを解析し、**struct/interface レベルのクラス図** と **パッケージ間の依存関係図** を Mermaid / PlantUML テキストとして出力する CLI ツール。
...
| 図の種類 | クラス図（struct/interface） + パッケージ依存図（循環依存の警告つき） | php-class-diagram の核を踏襲 |
...
diagoram [options] <dir>
  ...
  --disable-fields           クラス図にフィールドを描かない
  --disable-methods          クラス図にメソッドを描かない
