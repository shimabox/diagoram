...
func TestDiscoverDirsGeneratedFileModes(t *testing.T) {
	dir := t.TempDir()
	writeFiles(t, dir, map[string]string{
	...
	tests := []struct {
		...
		{name: "all", mode: GeneratedFilesAll, want: []string{"generated.go", "handwritten.go"}},
		{name: "exclude", mode: GeneratedFilesExclude, want: []string{"handwritten.go"}},
		{name: "only", mode: GeneratedFilesOnly, want: []string{"generated.go"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := discoverDirs(dir, ParseOptions{GeneratedFiles: tt.mode})
			if err != nil {
				t.Fatalf("discoverDirs: %v", err)
			}
			want := []dirFiles{{Dir: ".", Files: tt.want}}
			assertDirFiles(t, got, want)
		})
	}
}
